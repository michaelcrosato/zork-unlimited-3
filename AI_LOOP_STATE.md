...
- Main objective: Reduce late-game distraction after players reach lit Platform
  13 with the signal token and true-ending tools.
- Outcome: Fully prepared lit-platform players keep the signal-booth progression
  and optional Mara poster beat, while escape/backtrack choices are hidden once
  they would mostly distract from resolving the ledger. The escape ending
  remains available before the signal token is recovered.
- Evidence:
  ...
  - `npm run health` passes with 34 tests, validation clean, all 25 scenes
  ...
  - Manual CLI route confirmed a fully prepared `lit_platform` offers only
    ...
    100/100.
- Follow-up: The coverage strategy still reports 20 unfinished frontier samples;
  inspect whether those should be labeled separately from genuinely unfinished
  playthroughs.
- Risks:
  - Hiding escape after token recovery is intentional guidance, but the escape
    ending must remain reachable for players who light the platform before
    finding the token.
