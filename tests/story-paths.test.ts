...
const MORNING_STOP_OBJECTIVE = "Pull after the remembered morning stops answer in full.";
...
describe("demo story critical paths", () => {
  ...
  it("adds a threshold beat before the direct passenger manifest release", async () => {
    ...
    expect(observation.scene.text).toContain("each cleared inch becoming an invitation");
    expect(observation.state.flags.held_passenger_threshold).toBe(true);
    expect(observation.state.flags.confirmed_threshold_clearance).toBeUndefined();
    expect(observation.state.flags.heard_mara_goodbye).toBe(true);
    expect(observation.objectives).toEqual([THRESHOLD_INTERCOM_OBJECTIVE]);
  ...
  it("pays off listening to passenger echoes before opening the manifest doors", async () => {
    ...
    expect(observation.choices.map((choice) => choice.id)).toEqual([
      ...
      "seat_echoed_passengers_before_release",
      "listen_to_echoed_manifest_from_boarding",
      "reach_release_with_echoed_manifest"
    ]);

    const seatedEchoState = choose(story, state, "seat_echoed_passengers_before_release");
    observation = observe(story, seatedEchoState);
  ...
  it("lets checked door-echoes confirm seats without a speaker detour", async () => {
    const story = await loadStory("stories/demo.yaml");
    let state = initialState(story);
  ...
  it("adds a one-time Mara handoff beat after opening the passenger manifest", async () => {
    ...
    expect(rendered).toContain("  Morning stops:\n    4. Ask Mara what morning means");
    expect(rendered).toContain("  Shared room / release:\n    5. Make room in the third car");
    expect(rendered).toContain("  Thumbprint oath:\n    6. Carry the darkened thumbprint");
    expect(rendered).toContain("    7. Touch Mara's torn thumbprint");
    expect(rendered).toContain("  Thumbprint receipt:\n    8. Confirm Mara's darkened thumbprint");
    expect(rendered).toContain("  Mara and manifest:\n    9. Ask Mara to sign off");
    expect(rendered).toContain(
      "  Shared count:\n    10. Let the opened passengers finish Mara's count together"
    );
    expect(rendered).toContain(
      "  Counts / answers:\n    11. Keep listening as the opened passengers answer Mara"
    );
    expect(rendered).toContain("  Threshold holding:\n    13. Hold the third-car threshold");
  ...
  it("promotes Mara's opened-door handoff directly from the manifest hub", async () => {
    ...
    expect(firstDirectReleaseObservation.scene.ending).toBe(true);
    expect(firstDirectReleaseObservation.state.flags.saw_mara_manifest_handoff).toBe(true);
    expect(firstDirectReleaseObservation.state.flags.heard_mara_goodbye).toBe(true);
    expect(firstDirectReleaseObservation.scene.text).toContain("Mara is still mid-handoff");
    expectIdealScore(firstDirectReleaseObservation.score);
  ...
  it("adds an optional opened-manifest count before the passenger roll call", async () => {
    ...
    expect(observation.choices[2]?.label).toBe(
      "Let Mara's handoff reach the opened doors before pulling"
    );
    ...
    expect(choiceIds[4]).toBe("listen_to_passenger_morning_chorus");
    ...
    expect(choiceIds[7]).toBe("carry_opened_manifest_count_to_speaker");
    expect(observation.choices[7]?.label).toBe(
      "Carry Mara's opened count straight to the third-car speaker"
    );
    expect(observation.choices[7]?.choiceGroup).toBe("Manifest count");
    ...
    expect(observation.choices[8]?.choiceGroup).toBe("Shared count");
    expect(choiceIds[9]).toBe("board_with_opened_manifest_reviewed_count");
    expect(observation.choices[9]?.label).toBe(
      "Review Mara's opened count before carrying it to the speaker"
    );
    expect(choiceIds[10]).toBe("help_opened_passengers_gather");
    expect(observation.choices[10]?.label).toBe(
      "Help the opened passengers gather by helping one another board"
    );
    expect(choiceIds[11]).toBe("share_release_after_opened_passengers_gather");
    expect(observation.choices[11]?.label).toBe(
      "Pass the release hand to hand after the opened passengers gather"
    );
    expect(observation.choices[11]?.choiceGroup).toBe("Passenger gathering");
    expect(choiceIds[12]).toBe("ask_conductor_punch_from_opened_manifest");
    ...
    expect(observation.choices[14]?.label).toBe(
      "Ask the conductor to clear every opened door before boarding"
    );
    expect(observation.choices[14]?.choiceGroup).toBe("Passenger gathering");
    ...
    expect(choiceIds[16]).toBe("let_opened_lunch_tin_worker_set_pace");
    expect(observation.choices[16]?.label).toBe(
      "Let the lunch-tin worker set a count that includes himself"
    );
    expect(observation.choices[16]?.choiceGroup).toBe("Lunch tin count");
    expect(choiceIds[17]).toBe("pull_release_on_opened_lunch_tin_count");
    expect(observation.choices[17]?.label).toBe(
      "Carry the lunch-tin count to Mara until the worker counts himself"
    );
    expect(observation.choices[17]?.choiceGroup).toBe("Lunch tin count");
    expect(choiceIds[18]).toBe("let_opened_lunch_tin_worker_count_himself");
    expect(observation.choices[18]?.label).toBe(
      "Let the lunch-tin worker count himself before the opened release"
    );
    expect(observation.choices[18]?.choiceGroup).toBe("Lunch tin self-count");
    expect(choiceIds[19]).toBe("hold_opened_manifest_threshold");
    expect(observation.choices[19]?.label).toBe(
      "Hold the third-car threshold while Mara keeps the speaker open"
    );
    expect(choiceIds[20]).toBe("hold_and_confirm_opened_manifest_threshold");
    expect(observation.choices[20]?.label).toBe(
      "Hold the opened threshold while Mara carries the clearance"
    );
    expect(choiceIds[21]).toBe("listen_to_opened_threshold_from_manifest");
    expect(observation.choices[21]?.label).toBe(
      "Let Mara talk you through holding the opened threshold"
    );
    expect(choiceIds[22]).toBe("notice_manifest_thumbprint_from_opened_doors");
    ...
    expect(choiceIds[23]).toBe("carry_manifest_thumbprint_oath_from_opened_doors");
    expect(observation.choices[23]?.label).toBe(
      "Carry Mara's torn thumbprint oath straight to the third-car speaker"
    );
    expect(observation.choices[23]?.choiceGroup).toBe("Thumbprint oath");
    expect(choiceIds[24]).toBe("pull_release_with_manifest_thumbprint_oath_from_opened_doors");
    expect(observation.choices[24]?.label).toBe(
      "Let the opened passengers receive Mara's thumbprint oath before release"
    );
    expect(observation.choices[24]?.choiceGroup).toBe("Thumbprint receipt");
    expect(choiceIds[25]).toBe("confirm_manifest_thumbprint_receipt_from_opened_doors");
    expect(observation.choices[25]?.label).toBe(
      "Confirm the opened passengers receive Mara's thumbprint oath"
    );
    expect(observation.choices[25]?.choiceGroup).toBe("Thumbprint receipt");
    expect(choiceIds[26]).toBe("return_opened_manifest_mitten");
    expect(observation.choices[26]?.label).toBe(
      "Return the opened manifest's lost mitten to the child"
    );
    expect(choiceIds[27]).toBe("pair_opened_manifest_mittens");
    expect(observation.choices[27]?.label).toBe("Pair the child's mittens at the third-car frame");
    expect(observation.choices[27]?.choiceGroup).toBe("Keepsakes / memories");
    ...
    expect(choiceIds.indexOf("carry_opened_manifest_count_to_speaker")).toBeLessThan(
      choiceIds.indexOf("help_opened_passengers_gather")
    );
    ...
    expect(choiceIds.indexOf("help_opened_passengers_gather")).toBeLessThan(
      choiceIds.indexOf("share_release_after_opened_passengers_gather")
    );
    expect(choiceIds.indexOf("share_release_after_opened_passengers_gather")).toBeLessThan(
      choiceIds.indexOf("ask_conductor_punch_from_opened_manifest")
    );
    expect(choiceIds.indexOf("help_opened_passengers_gather")).toBeLessThan(
    ...
    expect(choiceIds.indexOf("ask_conductor_punch_from_opened_manifest")).toBeLessThan(
      choiceIds.indexOf("pull_release_on_opened_conductor_signal")
    );
    expect(choiceIds.indexOf("pull_release_on_opened_conductor_signal")).toBeLessThan(
      choiceIds.indexOf("confirm_opened_conductor_clearance")
    );
    expect(choiceIds.indexOf("confirm_opened_conductor_clearance")).toBeLessThan(
      choiceIds.indexOf("hold_opened_manifest_threshold")
    );
  ...
  it("lets opened-manifest players carry Mara's count straight to the speaker", async () => {
    const story = await loadStory("stories/demo.yaml");
    let state = initialState(story);
    ...
    expect(observation.scene.id).toBe("passengers_released");
    expect(observation.choices.map((choice) => choice.id)).toContain(
      "carry_opened_manifest_count_to_speaker"
    );

    state = choose(story, state, "carry_opened_manifest_count_to_speaker");
    observation = observe(story, state);
    ...
    expect(observation.state.flags.reviewed_open_manifest_count).toBe(true);
    expect(observation.state.flags.heard_mara_goodbye).toBe(true);
    expect(observation.state.flags.passengers_finished_reviewed_count).toBeUndefined();
    expect(observation.choices.map((choice) => choice.id)).toEqual([
      "let_passengers_finish_reviewed_count",
      "ask_who_reviewed_count_left_blank",
      "pull_release_before_reviewed_count_finishes",
  ...
  it("gives manifest-thumbprint receipt confirmation its own ending payoff", async () => {
    const story = await loadStory("stories/demo.yaml");
    let state = initialState(story);
  ...
  it("lets opened-manifest players confirm Mara's thumbprint receipt directly", async () => {
    const story = await loadStory("stories/demo.yaml");
    let state = initialState(story);
    ...
    expect(observation.scene.id).toBe("passengers_released");
    expect(choiceIds).toContain("confirm_manifest_thumbprint_receipt_from_opened_doors");
    expect(
      choiceIds.indexOf("confirm_manifest_thumbprint_receipt_from_opened_doors")
    ).toBeGreaterThan(choiceIds.indexOf("carry_manifest_thumbprint_oath_from_opened_doors"));
    expect(choiceIds.indexOf("confirm_manifest_thumbprint_receipt_from_opened_doors")).toBeLessThan(
      choiceIds.indexOf("return_opened_manifest_mitten")
    );

    state = choose(story, state, "confirm_manifest_thumbprint_receipt_from_opened_doors");
    observation = observe(story, state);
  ...
  it("lets opened manifest players carry the lunch-tin count to the speaker before release", async () => {
    const story = await loadStory("stories/demo.yaml");
    let state = initialState(story);
    ...
    expect(
      observation.choices.find((choice) => choice.id === "pull_release_on_opened_lunch_tin_count")
        ?.label
    ).toBe("Carry the lunch-tin count to Mara until the worker counts himself");
    expect(
      observation.choices.find((choice) => choice.id === "pull_release_on_opened_lunch_tin_count")
        ?.choiceGroup
    ...
    expect(observation.scene.text).toContain("Mara stops reading");
    expect(observation.scene.text).toContain("Pull the release while they are moving together");
    expect(observation.state.flags.helped_passengers_gather).toBe(true);
    expect(observation.state.flags.steadied_lunch_tin_worker).toBe(true);
    expect(observation.state.flags.set_lunch_tin_pace).toBe(true);
    expect(observation.state.flags.heard_gathered_passengers).toBe(true);
    ...
    expect(
      observation.choices.find((choice) => choice.id === "pull_release_after_lunch_tin_intercom")
        ?.label
    ).toBe("Pull while the lunch-tin count is moving together");
  ...
  it("lets manifest explorers follow the lunch-tin latch before crossing the platform", async () => {
    ...
    expect(observation.scene.id).toBe("passenger_lunch_tin_boarding");
    expect(observation.scene.text).toContain("clicks the latch");
    expect(observation.scene.text).toContain("emergency release waits");
    expect(observation.state.flags.helped_passengers_gather).toBe(true);
    expect(observation.state.flags.steadied_lunch_tin_worker).toBe(true);
    expect(observation.state.flags.set_lunch_tin_pace).toBe(true);
    expect(observation.choices.map((choice) => choice.id)).toContain(
  ...
  it("confirms conductor clearance directly from the opened passenger hubs", async () => {
    const story = await loadStory("stories/demo.yaml");
    const basePath = [
      "read_notice",
      "take_lantern_after_notice",
    ...
    expect(observation.scene.id).toBe("passengers_released");
    expect(choiceIds).toContain("pull_release_on_opened_conductor_signal");
    expect(choiceIds).toContain("confirm_opened_conductor_clearance");
    expect(choiceIds.indexOf("ask_conductor_punch_from_opened_manifest")).toBeLessThan(
      choiceIds.indexOf("pull_release_on_opened_conductor_signal")
    );
    expect(choiceIds.indexOf("pull_release_on_opened_conductor_signal")).toBeLessThan(
    ...
    expect(choiceIds.indexOf("confirm_opened_conductor_clearance")).toBeLessThan(
      choiceIds.indexOf("check_lunch_tin_count_from_opened_manifest")
    );
    ...
    openedState = choose(story, openedState, "pull_release_after_confirmed_conductor_clearance");
    observation = observe(story, openedState);
  ...
  it("lets opened-manifest players release answered names through Mara's speaker", async () => {
    ...
    expect(directChoice?.label).toBe(
      "Carry the answered names into the third car before the release"
    );
  ...
  it("lets opened-manifest players release as answered passengers board", async () => {
    ...
    const choiceIds = observation.choices.map((choice) => choice.id);
    const directChoice = observation.choices.find(
      (choice) => choice.id === "pull_release_with_answered_passengers"
    );
    ...
    expect(choiceIds.indexOf("pull_release_with_answered_passengers")).toBeLessThan(
      choiceIds.indexOf("board_and_check_answered_passengers")
    );
  ...
  it("lets answer listeners hear gathered passengers before the shared release", async () => {
    const story = await loadStory("stories/demo.yaml");
    let state = initialState(story);
  ...
  it("lets opened-manifest players confirm the lunch-tin roster proof directly", async () => {
    ...
    expect(observation.scene.id).toBe("passengers_released");
    expect(openedChoiceIds).toContain("confirm_opened_lunch_tin_roster_proof");
    expect(openedChoiceIds.indexOf("confirm_opened_lunch_tin_roster_proof")).toBeGreaterThan(
      openedChoiceIds.indexOf("call_lunch_tin_roster_from_opened_manifest")
    );

    state = choose(story, state, "confirm_opened_lunch_tin_roster_proof");
    observation = observe(story, state);
  ...
  it("lets platform explorers confirm the lunch-tin roster proof directly", async () => {
    const story = await loadStory("stories/demo.yaml");
    let state = initialState(story);
    ...
    expect(observation.scene.id).toBe("passenger_platform");
    expect(platformChoiceIds).toContain("confirm_platform_lunch_tin_roster_proof");
    expect(platformChoiceIds.indexOf("confirm_platform_lunch_tin_roster_proof")).toBeGreaterThan(
      platformChoiceIds.indexOf("ask_lunch_tin_worker_to_set_pace")
    );

    state = choose(story, state, "confirm_platform_lunch_tin_roster_proof");
    observation = observe(story, state);
  ...
  it("adds a manifest-specific platform beat after clearing Mara's ledger entry", async () => {
    ...
    expect(choiceIds).toEqual([
      ...
      "share_release_after_opened_passengers_gather",
      "ask_conductor_punch_from_opened_manifest",
      "pull_release_on_opened_conductor_signal",
    ...
    expect(renderedPlayerView).toContain(
      "  Shared count:\n    25. Board after the opened passengers finish the count together"
    );
    expect(renderedPlayerView.indexOf("  Shared count:")).toBeLessThan(
      renderedPlayerView.indexOf("  Lunch tin count:")
    );
    expect(renderedPlayerView).toContain(
      "  Lunch tin count:\n    28. Check the lunch-tin worker's passenger count before boarding"
    );
    expect(renderedPlayerView).toContain(
      "  Lunch tin self-count:\n    33. Let the lunch-tin worker count himself before the opened release"
    );
    expect(renderedPlayerView.indexOf("  Lunch tin count:")).toBeLessThan(
      renderedPlayerView.indexOf("  Lunch tin self-count:")
    );
    expect(renderedPlayerView).toContain(
      "  Lunch tin roster:\n    34. Read the lunch-tin worker's roster for the opened passengers"
    );
    ...
    expect(renderedPlayerView).toContain(
      "  Keepsakes / memories:\n    62. Return the opened manifest's lost mitten to the child"
    );
    ...
    expect(renderedPlayerView.indexOf("  Morning stops:")).toBeLessThan(
      renderedPlayerView.indexOf("  Board / release:")
    );
  ...
  it("lets the opened manifest names answer directly before boarding", async () => {
    ...
    const rollCallState = choose(story, state, "let_manifest_answers_become_final_roll_call");
    observation = observe(story, rollCallState);
  ...
  it("adds a one-time passenger morning chorus after opening the manifest", async () => {
    ...
    expect(observation.state.flags.heard_passenger_mara_signoff).toBeUndefined();
  ...
  it("confirms remembered morning stops directly from the opened passenger hubs", async () => {
    ...
    expect(observation.scene.id).toBe("passengers_released");
    expect(choiceIds).toContain("confirm_opened_manifest_morning_stops");
    expect(choiceIds.indexOf("listen_to_passenger_morning_chorus")).toBeLessThan(
      choiceIds.indexOf("confirm_opened_manifest_morning_stops")
    );
    expect(choiceIds.indexOf("confirm_opened_manifest_morning_stops")).toBeLessThan(
    ...
    expect(observation.scene.id).toBe("passenger_platform");
    expect(choiceIds).toContain("confirm_platform_morning_stops");
    expect(choiceIds.indexOf("listen_for_platform_morning_chorus")).toBeLessThan(
      choiceIds.indexOf("confirm_platform_morning_stops")
    );
